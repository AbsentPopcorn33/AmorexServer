...
class Commands {
	...
	checkPerms(command, socket) {
		...
		if (command.perms == perm.dev && permissions.class != 'developer') {
			command.status('fail').send('You do not have permission to use this command, you must be a developer (get owned lmfao)');
			return false;
		}
		return true;
	}
	...
	add(cmd, args, other, fun) {
		if (typeof cmd == 'object' && !Array.isArray(cmd)) {
			this.all.push(new Command(
				...
				cmd.args,
				cmd.doc,
				cmd.perms !== undefined ? cmd.perms : perm.dev,
				cmd.execute
			));
		} else {
			this.all.push(new Command(
...
module.exports = ({ Config, Events }) => {
	...
	commands.add(['entity', 'e', 'tag', 'id', 'playerId'], [0, 50], { doc: 'Get the entity id./nDoc: /e [USERNAME ...]', perms: perm.user}, ({ args, body, command }) => {
		if (args.count == 0) {
  ...
  /*commands.add(['id', 'playerid'], [0, 50],  [doc: ['Get the entity\nDoc: /e [USERNAME ...]']],
    perms: perm.user, 
     ({ args, body, command }) => {
		if (args.count == 0) {
			return command.send(`Current entity-id: ${tags.add(body)}`);
	...
	const teams = ['BLUE','GREEN','RED','PURPLE','YELLOW','ORANGE','BROWN','CYAN','ROOM','ENEMIES','RADIANT'];
	commands.add('team', [0, 2], { doc: `Set team for the selected entity\nString teams: ${teams.join(', ')}\nDoc: /team [TEAM] [WHO]`, perms: perm.token}, ({ args, body, command }) => {
  ...
  commands.add(['xp', 'score', 'scr', 'setvalue'], [0, 2], { doc: 'Set score for the selected entity\nDoc: /xp [VALUE] [WHO]' }, ({ args, body, command }) => {
		const score = args.getNumber(0);
		const entity = args.getEntity(1, body);
		if (!entity) return;
		if (!score) {
			return command.send(`Current ${entity.name}'s score: ${entity.skill.score}`);
		}
		const oldScore = entity.score;
		entity.skill.reset();
		while (entity.skill.score < score) {
			entity.skill.score += entity.skill.levelScore;
			entity.skill.maintain();
		}
		entity.refreshBodyAttributes();
		const delta = entity.skill.score - oldScore;
		command.send(`Changed ${entity.name}'s score ${oldScore} -> ${score} (${delta > 0 ? '+' : ''}${delta})`);
	});
  ...
  commands.add({
		...
		doc: [
			'Turn into a bot',
			'Doc: /ai true or false',
			'Tip: Set NAME if you want to edit the entity in future'
		],
		execute: ({ args, body, command }) => {
			const ai = args.getBoolean(1, true);
	...
	commands.add(['me', 'message', 'dm'], [1, 100], { doc: 'Send a message to entity\nDoc: /me WHO MESSAGE', perms: perm.user }, ({ args, command }) => {
	...
	const multipliers = ['acceleration', 'topSpeed', 'health', 'shield', 'regen', 'damage', 'penetration', 'range', 'fov', 'density', 'stealth', 'pushability', 'recoilReceived', 'size', 'scoreReceived'];
  ...
  /*commands.add(['serverstatus', 'gametime', 'servertime', 'uptime', 'status'], 0, { doc: 'the "How Are You" command.\nExample: /hru', perms: perm.user }, ({ command }) => {
		command.send('I am good. How are you?');
	});*/
