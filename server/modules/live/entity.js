...
class StatusEffect extends EventEmitter {
    constructor(duration = 0, multipliers = {}, tick = a => a) {
        ...
        this.scoreReceived = getValidated(multipliers, 'score', 'numbers', labelThing)
...
class Entity extends EventEmitter {
    ...
    define(defs, emitEvent = true) {
        ...
        if (set.RADIANCE != null) this.settings.radiance = set.RADIANCE;
        ...
        if (set.STAT_NAMES != null) this.settings.skillNames = {
            ...
            shield_regen: set.STAT_NAMES?.SHIELD_REGEN ?? 'Regeneration',
            shield_cap: set.STAT_NAMES?.SHIELD_CAP ?? 'Shield Resistance',
        };
    ...
    refreshBodyAttributes() {
        ...
        for (let i = 0; i < this.statusEffects.length; i++) {
            ...
            if (effect.scoreReceived != null) scoreReceivedMultiplier *= effect.scoreReceived
            if (effect.size != null) sizeMultiplier *= effect.size;
        }

        this.sizeMultiplier = sizeMultiplier;
        let speedReduce = Math.pow(this.size * 0.91 / (this.coreSize || this.SIZE), 1);
        ...
        this.health.set(((this.settings.healthWithLevel ? 0.3 * this.level : 0) + this.HEALTH) * this.skill.hlt * healthMultiplier);
        this.health.resist = 1 - 1 / Math.max(1, this.RESIST + this.skill.brst);
        this.shield.set(((this.settings.healthWithLevel ? 0.2 * this.level : 0) + this.SHIELD) * this.skill.shi, Math.max(0, ((this.settings.healthWithLevel ? 0.002 * this.level : 0) + 1) * this.REGEN * (this.skill.rgn * 0.3) * regenMultiplier));
        ...
        this.fov = fovMultiplier * this.FOV * 280 * Math.sqrt(this.size);
        this.density = densityMultiplier * (1 + 0.13 * this.level) * this.DENSITY;
        this.stealth = stealthMultiplier * this.STEALTH;
        this.pushability = pushabilityMultiplier * this.PUSHABILITY / this.DENSITY / 3;
        this.recoilMultiplier = this.RECOIL_MULTIPLIER * recoilReceivedMultiplier;
        this.scoreMultiplier = scoreReceivedMultiplier;
    ...
    get level() {
        return Math.min((this.levelCap ?? Config.LEVEL_CAP, this.skill.level) * this.scoreMultiplier);
    }
    get size() {
        return this.bond == null ? (this.coreSize || this.SIZE) * this.sizeMultiplier * (1 + this.level/70): this.bond.size * this.bound.size;
    }
    get mass() {
    ...
    contemplationOfMortality() {
        ...
        if (this.isDead()) {
            ...
            let jackpot = util.getJackpot(this.skill.score) / this.collisionArray.length/* * this.master.settings.radiance*/;
            ...
            for (let i = 0; i < this.collisionArray.length; i++) {
                ...
                if (instance.master.settings.acceptsScore) {
                    ...
                    instance.skill.score += this.master.settings.radiance * jackpot;
                }
