...
function incoming(message, socket) {
    ...
    switch (m.shift()) {
        case "k":
            ...
            if (m.length === 1) {
                ...
                if (socket.permissions) {
                    ...
                    log('logs',`[INFO] A socket ( REDACTED ) was verified with the token: ( REDACTED )`);
                } else {
            ...
            log('logs',`[INFO] A socket ( REDACTED ) has been welcomed to the server room. Waiting for spawn request.`);
            break;
        case "s":
            // spawn request
            util.log(`[INFO] A socket ( ${socket.ip} ) is asking for spawn request, checking all securities...`);
            log('logs',`[INFO] A socket ( REDACTED ) is asking for spawn request, checking all securities...`);
            ...
            log('logs',"[INFO] Passed the security, spawning player.");
            socket.party = m[4];
            ...
            log('logs',`[INFO] ${name == "" ? "An unnamed player" : m[0]} ${needsRoom ? "joined" : "rejoined"} the game on team ${socket.player.body.team}! Players: ${players.length}`);
            break;
        case "S":
            // clock syncing
            if (m.length !== 1) {
        ...
        case "M":
            ...
            log('logs',`[CHAT] ${player.body.name}: ${original}`)
